...
\subsection{Memory Management Frames (MM-frames)}
A memory management frame (MM-frame) is analogous to a stack frame but is independent of the C stack.
The procedure {\tt push\_MM\_frame()} pushes an MM-frame.  The procedure {\tt stack\_alloc(int nbytes)}
returns a pointer to a block of the given size allocated from the current MM-frame.  The procedure {\tt pop-MM-frame()}
...
Undo frames are similar to MM-frames but with the added feature of undoing effects tied to the frame.
...
undo frame frame returning the given expression as a value.  This is
implemented by first pushing a new MM-frame, copying the give expression value into the new MM-frame; poping the current undo frame;
copying the returned expression back into undo memory, and then popping the MM-frame holding the expression.  These transfers are all
