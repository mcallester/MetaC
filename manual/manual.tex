...
\item A connective.  A connective is a character string consisting of only ``connective characters''.
  ...
  \medskip
  \centerline{~\;\;\;\;\;\;\{\mtt{;}\} \{\mtt{,}\} \{$\epsilon$\}  \{\mtt{:}\} \{\mtt{@}\}
...
\begin{eqnarray*}
  ...
  E_\infty & ::= & \fopen S\;P^* \fclose \\
  S & ::= & \;\sym \;|\;\quot \;|\; \mathrm{SPECIAL} \;|\;\fopen \mathrm{SPECIAL}\;S\fclose
\end{eqnarray*}
...
strings of precedence $p$; and $\quot$ generates string quotations.  The nonterminal $\mathrm{SPECIAL}$ generates atoms for the three special characters
{\tt `}, $\$$ or $\backslash$.  ${\cal R}$ is the set of right-associative precedence levels and ${\cal L}$ is the set of left-associative levels.
Only the largest finite precedence level is left-associative. MetaC classifies all printable ASCII characters as being either alpha-numeric, connective, string quotations, parenthesis characters or one of three special characters.
