...
%% 
%% Perhaps introduce the type expptr here?
%% 
...
%% A single-element array in C is defined ``int y[1];''. The text here
%% seems to suggest ``int y[0];'' is proper C.  Could revise to
%% explain that we extend C in this way, or could revise MetaC to use
%% y[1] and abandon the y[0] syntax

Here assignments to $y[0]$ are allowed but assignments to $y$ are not --- assignments to array variables
...
%% I don't see how the tree structure is more apparent in this example
...
%% the general form should show the optional conditional tests for each pattern/body pair
%% I would add this but not certain how you want to do it.  Could be just
%% [.<test1>] before first colon, and [.<testn>] before second colon.
...
%%
%% might want to mention and provide macroexpand1 as well?
%%
...
with the command C-c C-c the text of the cell is passed to the C process from which a value is returned
...
%% mention the message ``Execution error'' for user to detect whether
%% ``p NIDE()'' or ``continue'' is needed
...
%% Note that I have added a couple more keystroke commands that are in
%% mc.el but not mentioned here.  I use them all the time:
%%    C-c b  load the entire buffer
%%    C-c B  load the buffer up to the current point
...
%% Did you mean to rename from #require to #include?
%% Also, as I wrote it, the <filename> could be a string or a symbol.
%%
%% A problem with using the name #require is that nothing is done to avoid reloading
%% A problem with using the name #include is that it is already in C but this is a bit different
...
%%
%% Note that setprop_int and getprop_int are not in base_decls.h
%%
...
%% ``sparsely'' perhaps should be ``loosely'' ?
%% 
...
%% 
%% is ``unwind protect'' a concept well understood outside the LISP community?
...
MetaC clears the base memory frame after every evaluation in the REPL and after every cell evaluation in the NIDE.  So it is important not to be using
...
%% 
%% the runtime of course also is proportional to the amount of undo
%% for clean_undo_frame, or computation time for computing the argument code for exp_from

%% Somewhere (here?) might be good to mention the GNU C extension for
...
%% I think it is important to show an example bracketing that reveals
%% the non-Lisp behavior of parenthesized lists (a (b c) d) ==> (<<a
%% (<b c>)> d>) and comment thereon.  This will cause untold confusion
%% among Lisp background folks even with this warning, worse without.
...
\caption{{\bf Examples of Reader Bracketings.} Bracketings are shown for the expression that results from reading the given strings.
  ...
  assumed to be right-associative and binary connective applications are assumed to be left-associative.  These conventions are used in the other examples.
...
%% 
%% It might be good to specify here the set of allowed characters overall given the recent fix.
%% 
...
%% 
%% Should connective characters also be non-quotation?
%% 
...
%% 
%% might want to mention how these ``lists'' are terminated
