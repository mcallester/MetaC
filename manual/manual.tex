...
MetaC also provides the procedures {\tt setprop\_int(expptr e, expptr p, int n)} and {\tt getprop(expptr e, expptr p, int default)}.
...
A MetaC memory frame is analogous to a stack frame but is independent of the C stack.
The procedure {\tt push\_memory\_frame()} pushes a memory frame.  The procedure {\tt stack\_alloc(int nbytes)}
returns a pointer to a block of the given size allocated from the current MM-frame.  The procedure {\tt pop-memory-frame()}
pops the current memory-management frame freeing all allocation done in that frame by resetting the appropriate free pointer.
MetaC pre-allocates the heap used by {\tt stack\_alloc}.
...
they had on entry to the frame.
...
The procedure {\tt value\_from\_undo(expptr exp)} exits the current
undo frame returning the given expression as a value.  This is
implemented by first pushing a new memory-frame, copying the give expression value into the new memory-frame; poping the current undo frame;
copying the returned expression back into undo memory, and then popping the memory-frame holding the expression.  These transfers are all
done in time proportional to the dag size of the expression returned.
...
\end{verbatim}
The executable {\tt expandA} is implemented entirely in $C$ but implements the backquote macro.  The file {\tt mcB.mc} implements {\tt ucase} using backquote.
The makefile also includes.
\begin{verbatim}
mcC.c : expandB mcC.mc
	 ./expandB mcC.mc mcC.c
\end{verbatim}
Here the file {\tt mcB.mc} implements additional macros using both backquote and ucase.  In the MetaC makefile this is continued up to mcE.mc and expandE.
MetaC provides the procedure
{\tt mcexpand(char * f1, char * f2)} which is used in the expandA and expandB commands above.  This procedure macro-expands the
...
have macro procedure pointers on their property lists, i.e., the expansion is done using whatever macros are defined in the current state.

The procedure {\tt mcexpand(char * f1, char * f2)} is implemented using the MetaC procedure {\tt file\_expressions(char * f)}.
This procedure takes a file name and returns a list of the expressions contained in that file.

\subsection{Macro Expansion Effects}

Macros expansion can also have effects.  The macro {\tt umacro} expands to a procedure definition of the macro expansion code but also
...
of initialization forms in its body.  In the REPL and IDE the intialization forms are run after the macro expansion of the input or cell is run.

\subsection{Preliminary Definitions for Macros}

A macro can also generate a preliminary definition.  It is possible to implement syntactic closures (lambda expressions) with a macro.
A closure consists of a procedure pointer bundled up with values for the free variables in the body of the lambda expression.  The lambda
...
the expression for constructing the closure data structure.  The construction of the procedure definition is a ``preamble'' to expression
returned as the expansion of the macro.  MetaC provides the primitive {\tt add\_preamble(expptr definition)} for creating a definition of a procedure
used in the expression returned by the macro.  In file expansion these preamble definitions are inserted into the output file before the
...
The MetaC procedure {\tt full\_macro\_expansion(expptr e)} returns a list consisting of the preamble expressions, the result of the macro expansion, and
...
We have the procedures related to memory frames.

  
We have the procedures related to undo frames.
...
We also have the procedures related to file expansion.
...
Finally we have lower level macro expansion procedures.

\begin{verbatim}
add_init_form(expptr stement);

add_preamble(expptr definition);

full_expansion(expptr e);
\end{verbatim}
