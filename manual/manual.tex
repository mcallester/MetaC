...
If an error occurs while reading the cell expression, or while macro-expanding the cell expression,
the NIDE will print {\tt "reader or expansion error"} and enter the gdb debugger.  The MetaC procedure {\tt berror(char *)}
enters the gdb debugger.  MetaC primitives call {\tt berror} when safety checks fail.
Whenever the gdb debugger is invoked entering
{\tt print NIDE()}, which can be abbreviate as {\tt p NIDE}, will return to the NIDE.

If an error occurs while attempting to compile the macro-expanded cell the NIDE will print {\tt "compilation error"}
and display the output of the compiler.
...
the NIDE will invoke the debugger.  In this case entering the command {\tt continue}
will continue the execution from the breakpoint.

Segmentation faults, and other types of errors, are trapped by gdb directly.  Again one can return
to the NIDE by entering {\tt p NIDE()}.
MetaC is designed to minimize the possibility of memory corruption
when returning from an error such as a segmentation fault.
...
\item[C-z a] moves to the begging of the current cell.
...
avoids a memory leak when returning from the an error to the REPL or
NIDE.
...
\begin{eqnarray*}
  ...
  \\
  \mathtt{(f\; b\;c)} & \Rightarrow & (\fopen \mathtt{f\;b\;c}\fclose)   \\
  \mathtt{(f\; (b)\;c)} & \Rightarrow & (\fopen \fopen \mathtt{f} \; (\mathtt{b})\fclose\;{\tt c}\fclose) \\
  \mathtt{(a\; (b)\;(c)\; d\;e)} & \Rightarrow & (\fopen \fopen \mathtt{a} \; (\mathtt{b})\;({\tt c})\fclose\;{\tt d\;e}\fclose)  \\
  \\
...
\caption{{\bf Examples of Reader Bracketings.} Bracketings are shown for the expression that results from reading the given strings.
  ...
  the brackets are ``phantoms'' that show the tree structure.  The
...
\item A connective.  A connective is a character string consisting of only ``connective characters''.
  ...
  intuitively represents the space character used as a connective.  The null connective and null arguments are discussed below.
